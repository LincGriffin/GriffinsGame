...
| **T2** ✅ | `feat/move-editor` | **Move editor dock** — a second EditorPlugin dock ("Moves") for add / duplicate / edit / delete of battle moves (kind picked from `MoveRepo.KINDS`, power, description) without hand-editing `tools/gen_moves.gd`. `MoveRepo` gained full CRUD (was read-only). Independent of the gameplay phases. | T |
| **T3** ✅ | `feat/powerup-editor` | **Power-up editor dock + data-driven power-ups** — power-ups became `PowerupData` resources (`assets/data/powerups/`, `gen_powerups.gd`); the chooser (`run.gd::_build_upgrade_options`) now draws from them instead of a hardcoded list. Third EditorPlugin dock ("Power-ups") for CRUD, with **portrait & sprite art upload** (`PowerupArt`, `assets/powerup_{portraits,sprites}/`). | 16, T2 |
...
| **18** ✅ | `feat/move-effects` | **Per-move audio + visual effects** — `MoveData` gains optional `sfx` / `vfx` **id** fields; battle plays them when a move occurs. `sfx` reuses `SfxLibrary` (blank → the per-kind `move_<kind>` sound); `vfx` is a self-contained **`CPUParticles2D`** PackedScene under `assets/vfx/<id>.tscn` (new `VfxLibrary` + `tools/gen_vfx.gd`), played at the target/caster. Effects are **referenced by id so moves share or swap them** (e.g. `strike` & `heavy` reuse `slash`); blank/missing = today's feedback. The Moves dock gets sfx/vfx fields + Browse upload (generalized `AssetLink.import_file`). Cosmetic only. | 3, 11, 12 |
| **19** ✅ | `feat/persistence` | **Persistence layer** — one `SaveManager` façade over pure-GDScript stores (**no SQLite / no native GDExtension**, which would break the headless pipeline): `GameConfig` (`ConfigFile`, `user://config.cfg`) makes **audio settings persist across launches** (applied at boot by `SoundManager`, saved on change by `SettingsMenu`); run history routes through the façade (`RunHistory` JSON unchanged behind it). **Save/resume is scaffolded**: `SaveGame` (Resource) schema + `SaveSlot` round-trip + `RunState.serialize_party`/`restore_party` (handles run-time **fused** monsters with no on-disk data) — built & unit-tested, not yet wired into gameplay. | 6, 14 |
| **20** | `feat/save-resume-wiring` (next) | **Save/resume wiring** — capture a live run into a `SaveGame` (map + cleared rooms + player cell + party + counters) on node resolution, and add a title-screen **"Resume"** that restores it. Pure integration on the Phase 19 data layer. | 19 |
| **21** ✅ | `feat/merge-offer` | **Merge is a post-capture offer** — after a won wild/elite fight, if the party has a **never-fused** member (`Combatant.is_fused`), an overlay offers to fuse the **newly captured** monster with a chosen partner (`RunState.merge_incoming` — party size unchanged). Below cap, declining recruits it as-is; **at cap, declining loses it**. Retires the old at-cap "pick any two" flow. On Merge, a self-contained **fuse animation** (cards converge → a `merge` `CPUParticles2D` burst + flash → the result reveals) plays, reusing the Phase 18 vfx system. `is_fused` persists via the Phase 19 save serialization. | 6, 18, 19 |
| **22** ✅ | `feat/move-cooldowns` | **Move balance + cooldown/charge properties** — `reflect` now takes **half** the hit and slams the **full** amount back; `guard` gains a **50% stun** on the foe. New per-move data properties (editable in gen_moves / the Moves dock): **`cooldown`** (turns unavailable after use — Strike 0, every other move 1) enforced **both sides**, and **`charge`** (a move that spends a turn charging before firing, off by default). The cooldown alone dropped the dominant "burst" move-spam strategy from ~50% → ~10% in `balance_sim`. | 3, 15, 18 |
| **23** ✅ | `fix/dynamic-monster-flags` | **Starter/wild/elite pools became data-driven** — `run.gd` reads `is_starter`/`tier`/`is_elite` live from `MonsterRepo.load_all()` instead of hardcoded `STARTER_ENEMIES`/`WILD_ENEMIES`/`ELITE_ENEMIES` const arrays, so toggling a flag via the Monsters dock takes effect immediately. Also fixed several dock bugs found while testing: "Array is in read-only state" adding a move to a freshly-created monster, a deleted-then-recreated monster "remembering" its old stats/art (stale `ResourceLoader` cache), and a fused **identity** result (e.g. `chicken+rat → goblin`) wrongly excluding itself from further fusion. | T, T2, 6 |
| **24** | `feat/fusion-editor` | **Fusion editor dock** — the fourth EditorPlugin dock ("Fusions"): monster-merge recipes became `FusionRecipeData` resources (`assets/data/fusions/`, `gen_fusions.gd`) instead of a hardcoded `FusionTable.TABLE` dict, edited via three monster-id pickers (Parent A / Parent B / Result) rather than a freely-typed id — a recipe's id is *derived* from its sorted parent pair, so a pair can only ever have one recipe. `FusionTable.lookup()` now reads the recipes live (memoised, like `Portraits`/`MapSprites`). | T, T3, 6 |
