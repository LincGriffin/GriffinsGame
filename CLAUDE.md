...
- **Monster merge (Phase 6 fusion rules; Phase 21 post-capture offer).** The fusion math is
  ...
  **max** of the two parents + a small bonus (`HP_MULT`/`ATK_BONUS`/`DEF_BONUS` — never the additive
  ...
  special parent pair (`scripts/data/fusion_table.gd`, reading `FusionRecipeData` resources from
  `assets/data/fusions/` — see the Fusion editor dock below — keyed by the two ids sorted; a pair
  CAN be the same id twice, e.g. `griffin`+`griffin`) becomes that **specific monster** (portrait/name), every
...
- **Marker tiles:** the tileset atlas is floor `(0,0)`, wall `(1,0)`, then walkable node markers
  ...
  custom data.
- **Height-gradient tint:** the floor/wall tiles are a single generated colour each, so
  `dungeon_view.gd::_apply_height_gradient` puts a `ShaderMaterial`
  (`assets/shaders/tile_gradient.gdshader`) on the TileMapLayer that washes them **cool near the
  bottom (entrance) → warm near the top (the higher, tougher nodes)**. The shader is **world-anchored**
  (a `MODEL_MATRIX`-transformed `VERTEX.y` varying, not screen space), so a tile keeps its colour as
  the camera scrolls; `y_top`/`y_bottom` are set to the map's world-Y extent. It **blends toward** the
  gradient colour rather than multiplying (a multiply desaturates the blue walls to grey at the warm
  end — caught in a screenshot). Only the tilemap is shaded; the marker glow/sprites are separate
  nodes and keep their colours.
...
- **Fusion editor** — `addons/fusion_editor/` (dock "Fusions"): add / edit / delete
  [[monster-merge-design]] recipes (which parent pair produces a specific result instead of a
  generic "Fused &lt;parent&gt;" blend) without hand-editing `scripts/data/fusion_table.gd`.
  Backed by `scripts/data/fusion_repo.gd` (`FusionRepo`) over `assets/data/fusions/*.tres`
  (`FusionRecipeData`: `parent_a`/`parent_b`/`result_id`, all monster ids), generated by
  `tools/gen_fusions.gd`. **Different shape from the other three docks:** a recipe's `id` isn't
  freely typed — it's *derived* from the sorted parent pair (`FusionRepo.id_for`, e.g.
  `"griffin_griffin"`), so a given pair can only ever have one recipe and there's no id text
  field, just three monster-id `OptionButton`s (Parent A / Parent B / Result, refreshed on every
  load same as the Monsters dock's move picker). "New" only clears the form — nothing is written
  until Save, once both parents and a result are chosen (`REPO.save()` re-derives and validates
  the id every time, so it also handles "I changed a parent" by deleting the old pair's file).
  `FusionTable.lookup()` now reads `FusionRepo.load_all()` (memoised — `clear_cache()` after any
  dock edit, same convention as `Portraits`/`MapSprites`) instead of a hardcoded `TABLE` dict.
