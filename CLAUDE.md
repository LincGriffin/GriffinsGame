...
- `assets/{sprites,tilesets,audio}` — art/audio. Tiles/sprites are the **Magna-Tiles** look
  (translucent primary-color "backlit plastic"; see `docs/DESIGN.md` → Art direction), generated by `gen_art.gd`.
  The map nodes/paths (`map_view.gd`) and battle backdrop (`gen_scenes.gd`) carry the same treatment in code.
...
- The **main scene is `scenes/map/run.tscn`** (`scripts/run.gd`), not the walkable room. A run:
  ...
  layered DAG that forks/reconnects and funnels to the Hydra) rendered by `scripts/map/map_view.gd`.
- Selecting a node resolves it: **battle** (wild → auto-recruit) / **elite** (tough fight → recruit +
  full heal) / **boss** / **heal** (full party) / **powerup** (teach a monster a new move, else +max HP) /
  **teleport** (jump ~2 rows ahead) / **room** (a walkable treasure room, below). Win the boss → YOU WIN;
  a party wipe → GAME OVER (press **R** → fresh run + new starter). Nothing persists between runs.
...
- **Deferred** (kept for later): a power-up / target chooser, and a rest-vs-treasure split.
