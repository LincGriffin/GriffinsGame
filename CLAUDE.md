...
- **Party battles, one active monster at a time.** There is **no "Hero"** — every fighter is a monster.
  ...
  Actions: Attack / Defend / Flee (no fleeing the boss).
- **Content is data-driven.** `MonsterData` (`scripts/data/monster_data.gd`) resources live in
  ...
  enemies and recruited allies. `Combatant` (`scripts/battle/combatant.gd`) is the runtime participant;
  its `compute_damage` is **static and pure** (unit-tested in `test_battle.gd`).
