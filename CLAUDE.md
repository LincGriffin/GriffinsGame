...
- **Party battles, one active monster at a time.** There is **no "Hero"** — every fighter is a monster.
  ...
  The command menu lists the active monster's **moves** — attack / guard / heal — plus Flee (no
  fleeing the boss).
...
- Winning a wild battle **auto-recruits** the defeated monster (up to `RunState.PARTY_CAP`, currently 5).
  Defeating the **boss (the Griffin)** wins the run; a party wipe shows GAME OVER (press **R** for a fresh
  run + new starter).
